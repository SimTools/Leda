...
#define __BFIELD__ 40.

#define __RT0DET__ 30.
#define __SIGMAXT0__ 0.0010
#define __SIGMAZT0__ 0.0010

#define __NSTEPS__ 118
#define __NWIRES__ 1
#define __RMIN__   36.2
#define __RMAX__   161.8
#define __RSTEP__  (__RMAX__-__RMIN__)/(__NSTEPS__-1)
#define __LHALF__  250.

#define __VDRIFT__ 5.e-3
#define __SIGMAX__ 0.015
#define __SIGMAZ__ 0.05
...
int main (Int_t argc, Char_t **argv)
{
   ...
   TH1D *hNDFPtr   = new TH1D("hNDF","NDF",500,0.,500.);
   TH1D *hChi2Ptr  = new TH1D("hChi2","Chi2",200,0.,400.);
...
#ifndef __T0DET__
   EXKalDetector detector(nlayers,lhalf,rmin,rstep);
#else
   EXKalDetector detector(nlayers,lhalf,rmin,rstep,rt0det);
#endif
#ifndef __MS_ON__
   detector.SwitchOffMS();	// switch off multiple scattering
#endif
   ...
   for (Int_t lyr = 0; lyr < nlayers && lyr >= 0; lyr += dlyr) {
       ...
       TKalMatrix h = ms.XvToMv(xx,side);
       Double_t   rphi = h(0,0);	            // r * phi 
       Double_t   d    = h(1,0);	            // drift length

       Double_t vdrift;
       Double_t rdet  = ms.GetR();
       if (rdet > rt0det) {
       ...
       hits.Add(new EXHit(ms,meas,dmeas,side,vdrift,xx,b));
#ifdef __DEBUG__ 
       cerr << " r= "    << setw(5) << setprecision(4) << ms.GetR()
            << " "
            << " rphi= "    << setw(7) << setprecision(4) << rphi
            << " drphi= "   << setw(4) << setprecision(2) << dx
            << " "
            << " d= "    << setw(5) << setprecision(4) << d 
            << " dd= "   << setw(2) << setprecision(2) << dz 
            << endl;
       cerr << setprecision(7);
#endif
       msPtrs = msPtr;
...
#ifdef __BACKWARD__
   hel1st.MoveTo(heltrk.GetPivot(),dfi);
#else
   ...
   svd(0,0) = 0.;
#if 0
   svd(1,0) = hel1st.GetPhi0();
   svd(2,0) = 1.e-3;
   svd(3,0) = 0.;
   svd(4,0) = hel1st.GetTanLambda();
#else
#if 0
   svd(1,0) = hel1st.GetPhi0();
   svd(2,0) = hel1st.GetKappa();
   svd(3,0) = 0.;
   svd(4,0) = hel1st.GetTanLambda();
#else
   Int_t i1 = 0;
...
cerr << " rt     = " << hel1st.GetRho() << endl;
cerr << " kappas = " << helstart.GetKappa() << endl;
cerr << " rs     = " << helstart.GetRho() << endl;
cerr << "---" << endl;
#endif
#endif
#endif
